...
CPlayer::CPlayer(HRESULT& hr, const HWND hwndVideo, const int iIndex) : m_lRefCount(1), m_hwndVideo(hwndVideo), m_iIndex(iIndex)
{
	hr = MFAllocateWorkQueue(&m_dwQueue);

	TRACE((L"CPlayer (%d) : %lu", iIndex, m_dwQueue));
	assert(hr == S_OK);
	assert(m_dwQueue != MFASYNC_CALLBACK_QUEUE_LONG_FUNCTION);
}
...
CPlayer::~CPlayer()
{
	...
	if(m_dwQueue != MFASYNC_CALLBACK_QUEUE_LONG_FUNCTION)
		LOG_HRESULT(MFUnlockWorkQueue(m_dwQueue));
}
...
HRESULT CPlayer::CreateInstance(const HWND hVideo, const int iIndex, CPlayer** ppPlayer)
{
	...
	pPlayer.Attach(new (std::nothrow)CPlayer(hr, hVideo, iIndex));
...
HRESULT CPlayer::GetParameters(DWORD* pdwFlags, DWORD* pdwQueue)
{
	...
	IF_FAILED_RETURN(pdwFlags == NULL || pdwQueue == NULL ? E_POINTER : S_OK);
	IF_FAILED_RETURN(m_dwQueue == MFASYNC_CALLBACK_QUEUE_LONG_FUNCTION ? MF_E_NOT_INITIALIZED : S_OK);
...
HRESULT CPlayer::Shutdown()
{
	...
	if(m_pSession != NULL){
		...
		if(ulMFObjects != 0)
		{
			TRACE((L"ulMFObjects = %lu (%d)", ulMFObjects, m_iIndex));
		}
